...
Streamlit アプリを起動します（コンテナ環境などからブラウザ接続する場合は `--server.address 0.0.0.0` を指定します）。

```bash
streamlit run nisa_sp500_timing/app.py --server.address 0.0.0.0 --server.port 8501
# または python 実行でも同等に起動できます
python nisa_sp500_timing/app.py
```
